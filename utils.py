...
class DataUtil(object):
    ...
    def random_split(instances, rates):
        ...
        for instance in instances:
            randn = random.random() # 生成一个0到1的随机符点数: 0 <= randn < 1.0
    ...
    def load_vector(file_path, ele_type):
        ...
        ele_type = eval(ele_type.lower()) # eval()：将字符串str当成有效的表达式来求值并返回计算结果
        vector = []
        f = open(file_path)
        for line in f:
            value = ele_type(line.strip()) # string.strip(s[, chars])：删除字符串头和尾的空白字符(包括\n，\r，\t这些)
            vector.append(value)
        f.close()
...
class MathUtil(object):
    ...
    def corr(x, y_train):
        ...
        """
        if MathUtil.dim(x) == 1:
            corr = pearsonr(x.flatten(), y_train)[0] # pearsonr(x,y)：计算特征与目标变量之间的相关度。输出：r： 相关系数 [-1，1]之间，p-value: p值
            if str(corr) == "nan":
                corr = 0.
        else:
            corr = 1.
    ...
    def aggregate(data, modes):
        ...
        aggregators = [getattr(np, m) for m in modes] # getattr() 函数用于返回一个对象属性值
    ...
    @staticmethod
    def cut_prob(p):
        p[p > 1.0 - 1e-15] = 1.0 - 1e-15 # 1e-15：10 的-15次方
        p[p < 1e-15] = 1e-15
        return p
...
class DistanceUtil(object):
    ...
    def edit_dist(str1, str2):
        try:
            ...
            d = 1. - SequenceMatcher(lambda x: x == " ", str1, str2).ratio() # 最长连续匹配子序列，忽略空格
        return d
    ...
    @staticmethod
    def jaccard_coef(A, B): # Jaccard相似系数。Jaccard系数值越大，样本相似度越高
